...
export const useEditorSlice = <T>(storeKey: string, initialState?: T) => {
  ...
  const addElement = useCallback(
    <K extends keyof T>(property: K, value: T[K] extends Array<infer I> | null ? I : never) => {
  ...
  const removeElement = useCallback(
    <K extends keyof T>(property: K, searchPayload: number | string | { [key: string]: any }) => {
  ...
  const getPropertySelector = useCallback(
    <K extends keyof T>(property: K) => {
      return (state: RootState): T[K] => get(state.editor[storeKey], property);
    },
    [storeKey],
  );
